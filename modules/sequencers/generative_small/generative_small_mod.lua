require "modules/sequencers/generative_small/generative_small_com"

class('GenerativeSmallMod').extends()

local modType = "GenerativeSmallMod"
local modSubtype = "midi"

function GenerativeSmallMod:init(x, y, modId)
	GenerativeSmallMod.super.init()
	
	if modId == nil then
		self.modId = "GenerativeSmallMod-" .. os.time() .. "_" .. math.random()
	else
		self.modId = modId
	end
	...
	self.component = GenerativeSmallCom(function() 
	end)
...
function GenerativeSmallMod:redrawCanvas()
	...
	love.graphics.printf(self.keyLabel, smallFont, 21 - 30, 4, 60, 'center')
	...
	--draw scale
	love.graphics.rectangle('line', 5, 70, self.width - 10, 24, 3)	
	love.graphics.printf(self.scaleLabel, smallFont, self.width/2 - 50, 75, 100, 'center')
...
function GenerativeSmallMod:canScroll() 
	return true
end

function GenerativeSmallMod:click(x, y, gX, gY)
	if y > (self.y + gY + 70) then
		dropdown:show(self.x + gX, self.y + 70 + gY, self.width, self.component:getScales(), function(index) 
...
function GenerativeSmallMod:scroll(oX, oY, dX, dY, gX, gY)
...
function GenerativeSmallMod:move(dX, dY)
	self.x = self.x + dX
	self.y = self.y + dY
end

function GenerativeSmallMod:visible(xOffset, yOffset)
	return true
end

function GenerativeSmallMod:focus()
	self.isFocused = true
end

function GenerativeSmallMod:unfocus()
	self.isFocused = false
end

function GenerativeSmallMod:contains(x, y, gX, gY)
	local collision = inBounds(x, y, self.x + gX, self.y + gY, self.width, self.height)
	return collision
end

function GenerativeSmallMod:setInCable(cable)
	self.component:setInCable(cable)
end

function GenerativeSmallMod:setOutCable(cable)
	self.component:setOutCable(cable)
end

function GenerativeSmallMod:tryConnectGhostOut(x, y, gX, gY, ghostCable)
...
function GenerativeSmallMod:tryConnectGhostIn(x, y, gX, gY, ghostCable)
...
function GenerativeSmallMod:update(deltaTime)
	self.component:update(deltaTime)
end

function GenerativeSmallMod:draw(xOffset, yOffset)
